...
# from module.bsp import BatchSpectralPenalizationLoss
# from module.afn import AdaptiveFeatureNorm
# from module.rsd import RepresentationSubspaceDistance
# from module.co_tuning import CoTuningLoss
...
class ConditionalDomainAdversarialLoss(nn.Module):
    ...
    def __init__(self, domain_discriminator: nn.Module, entropy_conditioning: Optional[bool] = False,
        ...
        # self.bsp = BatchSpectralPenalizationLoss()
        # self.afn = AdaptiveFeatureNorm(delta=1)
        # self.rsd = RepresentationSubspaceDistance(trade_off=0.1)
        # self.cotuning = CoTuningLoss()
    ...
    def forward(self, g_s: torch.Tensor, f_s: torch.Tensor, g_t: torch.Tensor, f_t: torch.Tensor) -> torch.Tensor:
        ...
        #bsp_loss = self.bsp(p, q)
        #afn_loss = self.afn(p) + self.afn(q)
        #co_loss =  self.cotuning(q, p)
        #coral_loss = self.coral(p, q)
