...
class ALBEF(nn.Module):
    def __init__(self,                 
        ...
        map_dim = 64
        image_map_layers = [nn.Linear(1024, 768), nn.Dropout(p=0.2)]
        text_map_layers = [nn.Linear(768, 768), nn.Dropout(p=0.2)]
        for _ in range(1, 2):
            image_map_layers.extend([nn.ReLU(), nn.Linear(768, map_dim), nn.Dropout(p=0.2)])
            text_map_layers.extend([nn.ReLU(), nn.Linear(768, map_dim), nn.Dropout(p=0.2)])
        ...
        pre_output_layers.extend([nn.Linear(5888, 5888), nn.ReLU(), nn.Dropout(p=0.1)]) #544->1088->1152
    ...
    def forward(self, image, text, targets, alpha=0, train=True):
        ...
        ##########
        align_loss = self.rsd(image_features, text_features)
        ...
        #####################
        features = torch.cat([corre_final, image_embeds, output], 1) #4096 + 64 + 64 =  4224 
