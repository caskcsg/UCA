...
class UnimodalDetection(nn.Module):
        def __init__(self, shared_dim=512, prime_dim = 64):
...
class ALBEF(nn.Module):
    def __init__(self,                 
        ...
        pre_output_layers.extend([nn.Linear(1088, 544), nn.ReLU(), nn.Dropout(p=0.1)])
        ...
        dim_list_image = [1024] * config['batch_size_train']
        self.mmdynamic =  MMDynamic(in_dim=dim_list_image, hidden_dim=[1024], num_class=2, dropout=0.5)
    ...
    def forward(self, image, text, targets, alpha=0, train=True):
        ...
        image_embeds = image_embed.pooler_output 
        ...
        features = torch.cat([text_final, img_final, corre_final], 1) #1024 + 32 + 32 = 1088
        #####################
        image_MMLoss, _ = self.mmdynamic(image_embed.last_hidden_state.transpose(0,1), targets)
        ...
        output['loss'] = self.cross_entropy_loss(logits, targets.float()) + 0.02 * image_MMLoss
