...
class ALBEF(nn.Module):
    def __init__(self,                 
        ...
        map_dim = 256
        image_map_layers = [nn.Linear(1024, map_dim),nn.ReLU(), nn.Dropout(p=0.2)]
        text_map_layers = [nn.Linear(768, map_dim), nn.ReLU(), nn.Dropout(p=0.2)]
        IT_map_layers = [nn.Linear(65536, 768), nn.ReLU(), nn.Dropout(p=0.2)]
        ...
        self.IT_map = nn.Sequential(*IT_map_layers)
        ...
        pre_output_layers.extend([nn.Linear(1088, 768), nn.ReLU(), nn.Dropout(p=0.1)]) #544->1088->1152
        ...
        self.mlp_encoder = MLPEncoder(activate='gelu', d_in=[77, 2, 256], d_hiddens=[[50, 1, 128], [10,1,32]], d_outs=[[50, 1, 128], [10,1,32]], dropouts=[0.5,0.5,0.5], bias=False, ln_first=False, res_project=[True,True])
    ...
    def forward(self, image, text, targets, alpha=0, train=True):
        ...
        image_embeds = self.image_encoder(image, return_dict=True)
        ...
        image_features = self.image_map(image_embeds.last_hidden_state) # [B, S, D] 256
        text_features = self.text_map(output.last_hidden_state)

        align_loss = self.cmd(image_features[:,0,:], text_features[:,0,:])
        #print(align_loss)
        ##########
        ima, tex = self.mulproj(image_embeds.pooler_output, output.pooler_output)
        skl = self.ambiguity_module(ima, tex)
        # skl = self.ambiguity_module(text_features, image_features)
        weight_uni = (1-skl)
        weight_corre = skl
        text_final = weight_uni * text_features[:,0,:]
        img_final = weight_uni * image_features[:,0,:]

        #############
        x = torch.stack([text_features,image_features[:,:77,:]], dim=2)
        x = self.mlp_encoder(x, mask=None)
        #print(x)
        #fused_features = x.mean(dim=1)

        features = x.reshape(text_features.shape[0], -1)

        text_final = F.normalize(text_final, p=2, dim=1) # [batch_size, d]
        img_final = F.normalize(img_final, p=2, dim=1) # [batch_size, d]

        IT_features = torch.bmm(img_final.unsqueeze(2), text_final.unsqueeze(1)) # [batch_size, d, d]
        IT_features = IT_features.reshape(IT_features.shape[0], -1)  # [batch_size, d*d]  #32*32=1024
        IT_features = self.IT_map(IT_features)
        #################
        corre_final = weight_corre * features
        corre_final = F.normalize(corre_final, p=2, dim=1) # [batch_size, d]

        # print(text_final.shape)
        # print(img_final.shape)
        # print(corre_final.shape)

        features = torch.cat([IT_features, corre_final], 1) #4096 + 64 + 64 =  4224 
        #####################
