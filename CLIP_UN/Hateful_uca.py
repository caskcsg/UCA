...
import sys
sys.path.append("../")
from models.model_uca import ALBEF
from models.vit import interpolate_pos_embed
from transformers import CLIPTokenizer, CLIPTextModel, CLIPProcessor, CLIPImageProcessor,CLIPTextConfig
import torchmetrics
...
def train(model, data_loader, optimizer, tokenizer,image_processor, epoch, warmup_steps, device, scheduler, config):
    ...
    for i,(images, text, targets) in enumerate(metric_logger.log_every(data_loader, print_freq, header)):
        ...
        text_inputs = tokenizer(text, max_length=77, padding='max_length', truncation=True, return_tensors="pt").to(device) 
