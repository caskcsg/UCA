...
import sys
sys.path.append("../")
from models.model_clip_hate_cen_p_768_bak_001 import ALBEF
from models.vit import interpolate_pos_embed
from transformers import CLIPTokenizer, CLIPTextModel, CLIPProcessor, CLIPImageProcessor
import torchmetrics
...
def train(model, data_loader, optimizer, tokenizer,image_processor, epoch, warmup_steps, device, scheduler, config):
    # train
    model.train()  
...
@torch.no_grad()
def evaluate(model, data_loader, tokenizer,image_processor,  device, config):
    # test
    model.eval()
...
def main(args, config):
    ...
    image_processor = CLIPImageProcessor.from_pretrained("/workspace/project/clip_large")
    tokenizer = CLIPTokenizer.from_pretrained("/workspace/project/clip_large")
    #### Model #### 
    ...
    for epoch in range(0, max_epoch):
        if not args.evaluate:
            ...
            train_stats = train(model, train_loader, optimizer, tokenizer, image_processor,  epoch, warmup_steps, device, lr_scheduler, config)  

        val_seen_stats = evaluate(model, val_seen_loader, tokenizer, image_processor, device, config)
        test_seen_stats = evaluate(model, test_seen_loader, tokenizer, image_processor, device, config)
...
if __name__ == '__main__':
    ...
    parser.add_argument('--text_encoder', default='/workspace/project/clip_large')
